...
namespace KevinCastejon.MultiplayerAPIExplorer
{
    public class LeaderboardPanel : UIPanel
    {
        ...
        [SerializeField] private TMP_InputField _getPlayerScoreIdInput;
        [SerializeField] private Button _getPlayerScoreBtn;
        [SerializeField] private TMP_InputField _getScoreIdInput;
        [SerializeField] private Button _getScoresBtn;
        [SerializeField] private TMP_InputField _addScoreIdInput;
        ...
        private async void GetPlayerScore()
        {
            ...
            try
            {
                entry = await LeaderboardsService.Instance.GetPlayerScoreAsync(_getPlayerScoreIdInput.text);
            }
            catch (Exception e)
            {
                LogException(e);
        ...
        private async void GetScores()
        {
            ...
            try
            {
                scores = await LeaderboardsService.Instance.GetScoresAsync(_getScoreIdInput.text);
            }
            catch (Exception e)
            {
                LogException(e);
        ...
        private async void AddScore()
        {
            ...
            try
            {
                entry = await LeaderboardsService.Instance.AddPlayerScoreAsync(_addScoreIdInput.text, int.Parse(_addScoreInput.text));
            }
            catch (Exception e)
            {
                LogException(e);
